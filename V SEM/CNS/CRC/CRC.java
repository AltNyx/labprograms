...
public class CRC {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) MSG[i] = sc.nextInt();

        // Evaluate the remainder
        int[] REM = generate(MSG, GEN);
        
        System.out.print("The remainder is: ");
        for(int i = 0; i < m-1; i++) System.out.print(REM[i]);
        
        System.out.print("\nThe CRC code is: ");
        for(int i = 0; i < n; i++) System.out.print(MSG[i]);
        for(int i = 0; i < m-1; i++) System.out.print(REM[i]);
    ...
    static int[] generate(int[] MSG, int[] GEN) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            if (MSB==1) {
                for(int j = 1; j < m; j++) REM[j-1] = REM[j]^GEN[j];
            } else {
                for(int j = 1; j < m; j++) REM[j-1] = REM[j]^0;
            }
